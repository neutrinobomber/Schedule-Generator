...
class Population {
  constructor(...params) {
    ...
    if (typeof firstArg === 'number' && typeof secondArg === 'object') {
  ...
  _initByTimetable(populationSize, timetable) {
    for (let individCount = 0; individCount < populationSize; individCount += 1) {
  ...
  shuffle() {
    for (let i = this._population.length - 1; i > 0; i -= 1) {
