...
class GeneticAlgorithm {
  constructor(populationSize, mutationRate, crossoverRate, elitismCount, tournamentSize) {
  ...
  selectParent(population) {
    ...
    population.shuffle();
    for (let i = 0; i < this._tournamentSize; i += 1) {
  ...
  mutatePopulation(population, timetable) {
    ...
    for (
      ...
      for (let geneIndex = 0; geneIndex < individual.getChromosomeLength(); geneIndex += 1) {
  ...
  crossoverPopulation(population) {
    ...
    for (
      ...
      if (this._crossoverRate > Math.random() && populationIndex >= this._elitismCount) {
        const offspring = new Individual(parent1.getChromosomeLength());
        const parent2 = this.selectParent(population);

        for (let geneIndex = 0; geneIndex < parent1.getChromosomeLength(); geneIndex += 1) {
          if (Math.random() < 0.5) {
